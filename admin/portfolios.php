...
<div id="content-wrapper">
    <div class="container-fluid">
        ...
        <table class="table table-bordered">
            ...
            if(isset($_POST["edit_portfolio"])){
                ...
                if(empty($portfolio_image_sm)){
                    ...
                    $select_image_sm = mysqli_query($conn,$query3);
                    while($row = mysqli_fetch_array($select_image_sm)){
                        $portfolio_image_sm = $row["portfolio_img_sm"];
                    }

                }
                if(empty($portfolio_image_bg)){
                    $query4 = "SELECT * FROM portfolios WHERE portfolio_id = '$_POST[portfolio_id]'";
                    $select_image_bg = mysqli_query($conn,$query4);
                    while($row = mysqli_fetch_array($select_image_bg)){
                        $portfolio_image_bg = $row["portfolio_img_bg"];
                    }
                ...
                $query2 = "UPDATE portfolios SET portfolio_name = '$portfolio_name', portfolio_category = '$portfolio_category', portfolio_img_sm = '$portfolio_image_sm', portfolio_img_bg = '$portfolio_image_bg' WHERE portfolio_id = '$_POST[portfolio_id]'";

                $edit_portfolio_query = mysqli_query($conn,$query2);
                header("Location: portfolios.php");
            ...
            $sql_query = "SELECT * FROM portfolios ORDER BY portfolİo_id DESC";
        ...
        if(isset($_GET["delete"])){
            ...
            $query = "DELETE FROM portfolios Where portfolio_id={$del_portfolio_id}";
